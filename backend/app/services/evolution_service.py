...
class EvolutionAPI:
    def __init__(self, api_key: str, base_url: str):
        ...
        self.base_url = validate_outbound_url(
            base_url,
            allow_private=settings.EVOLUTION_ALLOW_PRIVATE_TARGETS,
        )
    ...
    def send_message(
        ...
        try:
            ...
            return {
                "success": False,
                "error": "Falha de comunicação com a Evolution API.",
            }
...
def save_evolution_config(
    ...
    db.query(EvolutionConfig).filter(
        EvolutionConfig.owner_id == owner_id
    ).update(
        {
            "active": False,
    ...
    # A API key entra criptografada no banco.
...
def _build_evolution_api(
    config: EvolutionConfig,
) -> EvolutionAPI:
    """
    Centraliza a descriptografia da credencial.

    A chave só existe em texto puro na memória pelo tempo
    necessário para criar o cliente da Evolution API.
    """
...
def test_evolution_connection(
    ...
    api = _build_evolution_api(config)

    return api.get_instance_status(
...
def send_whatsapp_message(
    ...
    if not config:
        return {
            "success": False,
            "error": "Evolution API não configurada. Verifique as configurações.",
        }
